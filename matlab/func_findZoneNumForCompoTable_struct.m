...
function [tableActivity, tableZoned] = func_findZoneNumForCompoTable_struct(tableActivity, tableZoned, ...
...
for lev = 1:nLevel-1  % UPDATED0912 -------------------------------------------
    for z = 1:nZ
        for class = 1:nClass
            loc = (lev-1)*nZ*nClass + (z-1)*nClass + class;
            tableZoned{loc,'Floor'} = lev-1;  % UPDATED0912 -------------------------------------------
            tableZoned{loc,'Zone'} = z;
            tableZoned{loc,'ActName'} = strcat("Mount ",string(compoClass(class)),"_F",num2str(lev-1),"_Z",num2str(z));
